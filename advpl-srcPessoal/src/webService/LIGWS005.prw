...
WSSERVICE LIGWS005 Description "WS Cadastro Cliente"
...
WSDATA _DADOS7 AS ITENSLIST7 OPTIONAL // alteração realizada aqui - estava ITENSLIST
WSDATA _DADOS8 AS ITENSLIST8 OPTIONAL // alteração realizada aqui - estava ITENSLIST

WSMETHOD LIGWS5GR DESCRIPTION "Grava Dados Do Cliente" 
...
WSSTRUCT LISTA7
  WSDATA DADOS7 AS STRING
ENDWSSTRUCT

WSSTRUCT ITENSLIST7
  WSDATA REGISTROS7 AS ARRAY OF LISTA7
ENDWSSTRUCT

WSSTRUCT LISTA8
  WSDATA DADOS8 AS STRING
ENDWSSTRUCT

WSSTRUCT ITENSLIST8
  WSDATA REGISTROS8 AS ARRAY OF LISTA8
ENDWSSTRUCT
...
PROCESSO:
1) Mapeamento das posicoes dos arrays DADOS7 e DADOS8, que são os arrays de telefone e endereco, isso é apenas um açucar-sintatico.
...
7)
*/
WSMETHOD LIGWS5GR WSRECEIVE CPESSOA,CCGC,CNOME,CEMAIL,CDTNASC,CINSCR,CRG,CCONTAT,CCPFCON,CTIPO,DDDCLI,TELCLI,DDDCELCL,CELCLI,CODSU5,CDLJCLI,_DADOS7,_DADOS8 WSSEND COK WSSERVICE LIGWS005                    	
	Local codCont := ""
	//Abaixo, o mapeamento das posicoes de chegada do array _DADOS8, array de endereços
	...
	// para atualizar, dessa forma caso seja preciso atualizar os vetores DADOS7 e DADOS8 será bem mais facil.
	Local peCep := 1     //CEP
	...
	//Abaixo, o mapeamento das posicoes de chegada do array _DADOS7, array de telefones
	Local ptDDDI := 1    //DDI
   	Local ptDDD  := 2    //DDD
	...
	Local ptTipo2 := 9    //Linha Nova, Telefone, Portabilide
   	Local ptComp := 8    //Complemmento
	Local ptOwner := 8   //Titular da linha, nome do cliente
	Local ptCpfOw  := 8  //Cpf do titular da linha
	...
	::COK := "- "		
	CONOUT("ENTROU LIGWS5GR DO NAKAO."+CCGC+"."+TIME()+" - "+CCGC)	
	...
	//Os dois laços abaixo validam os campos envolvidos nos arrays DADOS7 e DADOS8.
	CONOUT("AGB - Telefones do Array") 
	CONOUT("Ordem esperada: DDI, DDD, Telefone, Tipo(1=comercial;2=residencial;3=fax comercial;4=fax residencial;5=celular),Padrao(1=Sim;2=Nao),Operacao(A=ALTERACAO, N=NOVO),codTelefone, vazio, tipo2(linha nova, telefone ou portabilidade)")	
	for j:=1 to len(::_DADOS7:REGISTROS7)
		if(ALLTRIM(::_DADOS7:REGISTROS7[j]:DADOS7)<>"")	
			dadosTel := StrTokArr(ALLTRIM(::_DADOS7:REGISTROS7[j]:DADOS7),";")
			CONOUT(::_DADOS7:REGISTROS7[j]:DADOS7)
	...
	for i:=1 to len(::_DADOS8:REGISTROS8)
		if ALLTRIM(::_DADOS8:REGISTROS8[i]:DADOS8)<>""
			dadosEnd := StrTokArr(ALLTRIM(::_DADOS8:REGISTROS8[i]:DADOS8),";")
			CONOUT(::_DADOS8:REGISTROS8[i]:DADOS8)
			if(len(endPadrao)=0)
			...
			if( len(u_dAgaVld("SA1", SUBSTR(CCGC,1,9)+IF((len(CCGC)=11), "001", SUBSTR(CCGC,10,3)),dadosEnd[peEstado],dadosEnd[peTipo], dadosEnd[pePadrao] ,dadosEnd[peBairro],"",dadosEnd[peMun], dadosEnd[peTipo2] ) )>0) //Chama a função de validação de endereco, LGDAOAGA.prw
				...
				::COK := "LISTA DE ENDERECOS CONTEM PELO MENOS UM ENDERECO INVALIDO"
				return .F.
			endif
		endif
	next
	...
	nkVldCli := u_DSa1Vld(CCGC, CNOME, endPadrao[3], endPadrao[5], endPadrao[1], CEMAIL, "", endPadrao[6], endPadrao[2], DDDCLI,TELCLI , DDDCELCL, CELCLI, "1030001", "105", CDTNASC, CTIPO, CPESSOA )
	if len(nkVldCli)>0
		for i:=1 to len(nkVldCli)
			::COK := u_barraN("Ops! Alguns campos apresentaram problemas no cadastro, segue:")
			::COK := ::COK+u_barraN(nkVldCli[i])//Funcao no lignkut.prw
		next
		return .F.
	endif
	...
	IF DBSEEK(xFilial("SA1")+CDLJCLI) //Se o cliente já for cadastrado, entramos no caso da alteração
		...
		BEGIN TRANSACTION			
			...
			for i:=1 to len(::_DADOS8:REGISTROS8) //ALTERA/CADASTRA OS ENDERECOS
				if ALLTRIM(::_DADOS8:REGISTROS8[i]:DADOS8)<>""
					dadosEnd := StrTokArr(ALLTRIM(::_DADOS8:REGISTROS8[i]:DADOS8),";")
			...
			for j:=1 to len(::_DADOS7:REGISTROS7)//ALTERA/CADASTRA OS TELEFONES
				if(ALLTRIM(::_DADOS7:REGISTROS7[j]:DADOS7)<>"")		
				    dadosTel := StrTokArr(ALLTRIM(::_DADOS7:REGISTROS7[j]:DADOS7),";")
		...
		BEGIN TRANSACTION
			...
			for i:=1 to len(::_DADOS8:REGISTROS8)//CADASTRA OS ENDERECOS
				if ALLTRIM(::_DADOS8:REGISTROS8[i]:DADOS8)<>""
				    dadosEnd := StrTokArr(ALLTRIM(::_DADOS8:REGISTROS8[i]:DADOS8),";")
			...
			next	
			/*		
			for j:=1 to len(::_DADOS7:REGISTROS7)//CADASTRA OS TELEFONES
				if(ALLTRIM(::_DADOS7:REGISTROS7[j]:DADOS7)<>"")		
					dadosTel := StrTokArr(ALLTRIM(::_DADOS7:REGISTROS7[j]:DADOS7),";")
	...
	::COK := "CADASTRO DO CLIENTE EFETUADO COM SUCESSO!"
	CONOUT("FIM LIGWS005GR")
RETURN .T. 