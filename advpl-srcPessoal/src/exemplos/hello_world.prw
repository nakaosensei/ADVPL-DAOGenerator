...
cTexto := ADA->ADA_UDSCAN
	...
	DEFINE MSDIALOG oDlgMC TITLE "Cancelamento do contrato : " + ADA->ADA_NUMCTR  FROM 000, 000  TO 390, 600 COLORS 0, 16777215 PIXEL
		...
		@ 040, 010 SAY oSay1 PROMPT "Isento?" SIZE 185, 014 OF oDlgMC FONT oFont1 COLORS 0, 16777215 PIXEL
		 cCombo1:=  aItems1[1]      
		 oCombo1 := TComboBox():New(052,010,{|u|if(PCount()>0,cCombo1:=u,cCombo1)},; 
	     aItems1,100,20,oDlgMC,,; 
	     ,,,,.T.,,,,,,,,,'cCombo1')   
		 
		@ 80,10 Get cTexto Size 280,70 MEMO of oDlgMC Pixel
...
Local oFont1 := TFont():New("MS Sans Serif",,022,,.F.,,,,,.F.,.F.)
	...
	DEFINE MSDIALOG oDlgME TITLE "Multa + pendencias " FROM 000, 000  TO 390, 600 COLORS 0, 16777215 PIXEL
		...
		oCheck1Me:bWhen := {|| .T.  }
		@ 032, 023 MSGET oGet1Me VAR cGet1Me SIZE 075, 010 OF oDlgME COLORS 0, 16777215 PIXEL PICTURE "@E"
		
		@ 020, 221 say "Total" OF oDlgME FONT oFont1 COLORS 0, 16777215 PIXEL
		@ 032, 221 MSGET oGet2Me VAR cGet2Me SIZE 075, 010 OF oDlgME COLORS 0, 16777215 PIXEL PICTURE "@E"
		...
		oCheck2Me:bWhen := {|| .T.  }
		@ 057, 023 MSGET oGet3Me VAR cGet3Me SIZE 075, 010 OF oDlgME COLORS 0, 16777215 PIXEL PICTURE "@E"
