...
User Function mBrwADA3()
	...
	*/
	...
	elseif(AllTRIM(tpUsr)=="2")//Se for supervisor
		enableW := .T.
		AADD(aRotina, { "Contrato Regularizado",    "U_MBADA3H"   , 0, 3 })
	endif
	restarea(_areaS)
	if(enableW = .F.)
...
User function MBADA3C() 
	...
	IF !EMPTY(aBrowse)
		...
		TButton():New( 172, 50,  "O.S", oDlg,{|| U_MBADA3D(.T.,aBrowse[oBrowse:nAt,02], _nCtr) },40,010,,,.F.,.T.,.F.,,.F.,,,.F.) 			
		...
		TButton():New( 172, 200, "Enviar Instalação", oDlg,{||  U_MBADA3D(.F.,aBrowse[oBrowse:nAt,02], _nCtr) },60,010,,,.F.,.T.,.F.,,.F.,,,.F.) 
...
User function MBADA3D(_Visu,_numOs,_nCtr)
	Local _aAB6 := AB6->(getarea())	
	Local _stay := .F.
	dbselectarea("AB6")
	dbsetorder(1)
...
User function MBADA3G(_numOs)
	//FWExecView('Inclusao por FWExecView','AB9_MVC', MODEL_OPERATION_INSERT, , { || .T.},{ || .T.} , , )
	Local codCli
	Local lojCli
	Local produto
	Local savedOS
	Local codPro
	Local _areaD := getarea()
...
//Contrato Regularizado tirar da listagem
User function MBADA3H()
	Local _area := getarea()
	Local _aADA := ADA->(getarea())
	IF msgyesno("Tem certeza que deseja marcar o Contrato como regularizado, fazendo isto o contrato não aparecerá mais na listagem?") 
		RECLOCK("ADA",.F.)
			ADA->ADA_UCANCE := ' '			
		MSUNLOCK()
	endif
