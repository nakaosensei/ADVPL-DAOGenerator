...
User Function AT450GRV()
	...
	email := ALLTRIM((cAliasAB9)->A1_EMAIL)
	os := ALLTRIM((cAliasAB9)->AB6_NUMOS)
	nome := ALLTRIM((cAliasAB9)->A1_NOME)
	atdt := ALLTRIM((cAliasAB9)->AB6_ATEND)
	flag := 1
	msg := "Olá "+nome+", a OS de nro "+os+" aberta por você teve seu atendimento concluido, o atendente da sua ordem é o "+atdt+"."+ Chr(13) + Chr(10) +u_barraN("Abaixo, estão listados os laudos de solução para os itens da OS em questao.:")
	WHILE !EOF()
		tipo := ALLTRIM((cAliasAB9)->AB7_TIPO)
		item := ALLTRIM((cAliasAB9)->AB7_ITEM)		
		if(tipo<>"5")
			flag := 0
		endif
		msg += "Item: "+item+" Solução:"+u_barraN(MSMM(AB7->AB7_MEMO3,35,,,3,,,"AB7","AB7_MEMO3"))+Chr(13) + Chr(10)
		DBSKIP()
	ENDDO
	if(flag=1)
