...
WSSERVICE LIGWS006 Description "WS Televendas Ligue Cianorte"
...
WSDATA _DADOS10 AS ITENSLIST10 OPTIONAL
WSDATA _DADOS9 AS ITENSLIST9 OPTIONAL
WSDATA _DADOS14 AS ITENSLIST14 OPTIONAL
WSMETHOD LIGWS6GR DESCRIPTION "Grava DADOS10 Do Televendas"

ENDWSSERVICE

WSSTRUCT LISTA10
  WSDATA DADOS10 AS STRING
ENDWSSTRUCT

WSSTRUCT ITENSLIST10
  WSDATA REGISTROS10 AS ARRAY OF LISTA10
ENDWSSTRUCT

WSSTRUCT LISTA9
  WSDATA DADOS9 AS STRING
ENDWSSTRUCT

WSSTRUCT ITENSLIST9
  WSDATA REGISTROS9 AS ARRAY OF LISTA9
ENDWSSTRUCT

WSSTRUCT LISTA14
  WSDATA DADOS14 AS STRING
ENDWSSTRUCT

WSSTRUCT ITENSLIST14
  WSDATA REGISTROS14 AS ARRAY OF LISTA14
ENDWSSTRUCT
...
ฑฑบPrograma  ณLIGWS6GR  บAutor Daniel Gouvea         บ Data ณ 13/02/2017  บฑฑ
ฑฑฬออออออออออุออออออออออสออออออออออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ Grava DADOS10 Televendas                                     บฑฑ
ฑฑบ          ณ                                                            บฑฑ
...
WSMETHOD LIGWS6GR WSRECEIVE CCGCLI,COPERADOR,CONDPG,CVEND,CTIPOCON,CTIPOCLI,COPERACAO,CMIDIA,CMKT,COCORREN,CPRAZO,CVALIDCTR,CDIAFECH,COBSLRD,CCLIASSYN,_DADOS10,_DADOS9,_DADOS14 WSSEND COK WSSERVICE LIGWS006
	LOCAL NCAMPOS := 0
	LOCAL CCAMPO  := ""
	LOCAL ACAMPOS := {}
	LOCAL NCONT   := 0
	...
	Local paTipo2 := 8    //Linha Nova, Telefone, Portabilide
   	Local paComp := 9    //Complemmento
	Local paOwner := 9   //Titular da linha, nome do cliente
	Local paCpfOw  := 9  //Cpf do titular da linha	
	
	//Abaixo, o mapeamento das posicoes de chegada do array _DADOS10, array de telefones
	...
	//Abaixo, o mapeamento das posicoes de chegada do array _DADOS10, array de itens
	Local piCodPro  := 1 
	Local piQtde    := 2
	Local piVlrUnit := 3
	Local piVlrCheio := 4
	...
	::cOk := "OK"
	CONOUT("ENTROU LIGWS6GR ."+CCGCLI+"."+TIME())
	...
	IF DBSEEK(XFILIAL()+ALLTRIM(CCGCLI))
		...
		for j:=1 to len(::_DADOS9:REGISTROS9)
			if(ALLTRIM(::_DADOS9:REGISTROS9[j]:DADOS9)<>"")	
				dadosTel := StrTokArr(ALLTRIM(::_DADOS9:REGISTROS9[j]:DADOS9),";")
				CONOUT(::_DADOS9:REGISTROS9[j]:DADOS9)
		...
		for j:=1 to len(::_dados14:Registros14)
			if(ALLTRIM(::_dados14:Registros14[j]:dados14)<>"")	
				dataTel := StrTokArr(ALLTRIM(::_dados14:Registros14[j]:dados14),";")
				CONOUT(::_dados14:Registros14[j]:dados14)
		...
		CONOUT("ORDEM ESPERADA ITENS: codPro,qtde, valorUnitario,valorCheio,Desconto,valorMensal,qtMesesIni,qtMesesCobranca,codEndInstalacao,numCtr,itemCtr,observacao")
		for nI := 1 to Len( ::_DADOS10:REGISTROS10 )
			if(ALLTRIM(::_DADOS10:REGISTROS10[nI]:DADOS10)<>"")
				_ITENS := StrTokArr( ::_DADOS10:REGISTROS10[nI]:DADOS10, ";" )						
				CONOUT(::_DADOS10:Registros10[nI]:DADOS10)						
			endif
		next	
			
		if Len( ::_DADOS10:REGISTROS10 )>0											
			DBSELECTAREA("SUA")
			DBSETORDER(1)
			...
			for j:=1 to len(::_DADOS9:REGISTROS9)//CADASTRA OS TELEFONES LINHA NOVA/PORTABILIDADE
				if(ALLTRIM(::_DADOS9:REGISTROS9[j]:DADOS9)<>"")		
					dadosTel := StrTokArr(ALLTRIM(::_DADOS9:REGISTROS9[j]:DADOS9),";")
			...
			next		
			for j:=1 to len(::_dados14:Registros14)//CADASTRA OS TELEFONES
				if(ALLTRIM(::_dados14:Registros14[j]:dados14)<>"")		
					dataTel := StrTokArr(ALLTRIM(::_dados14:Registros14[j]:dados14),";")
