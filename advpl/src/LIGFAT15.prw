...
Static Function RunReport(Cabec1,Cabec2,Titulo,nLin)
	...
	elseif val(MV_PAR02) <= 10
		...
		mescomp1 := alltrim(str(val(MV_PAR02)-1))
		anocomp1 := alltrim(MV_PAR03)
	endif
	...
	for i:=1 to len(aDadosV)
		...
		enddo
		TEMP->(DBCLOSEAREA())
		//regra total de vendas menor de 20
		if 20 > aDadosV[i,11] + aDadosV[i,13]+ (aDadosV[i,12]/2) .and. vendTercei != 'S'
			aDadosV[i,9] := 0
		endif
	next i
	//VENDA INCREMENTO
	for i:=1 to len(aDadosI)
		...
		enddo
		TEMP->(DBCLOSEAREA())
		// regra vendas menor que 20 nao comissiona
		if 20 > aDadosI[i,11] + aDadosI[i,13]+ (aDadosI[i,12]/2) .and. vendTercei != 'S'
			aDadosI[i,8] := 0
		endif
	next i
	//VENDA RENOVAวรO
	for i:=1 to len(aDadosR)
		...
		TEMP->(DBCLOSEAREA())
		//regra vendas menor que 20 nao comissiona
		if 20 > aDadosR[i,12] + aDadosR[i,14]+ (aDadosR[i,13]/2) .and. vendTercei != 'S'
			aDadosR[i,8] := 0
		endif
	next i
	...
	if(tpUsr == '2') //Gerente
		for i:=1 to len(aDadosVGE)
			...
			TEMP->(DBCLOSEAREA())
			//REGRA VENDAS MENOR QUE 20 NAO COMISSIONA
			if 20 > aDadosVGE[i,11] + aDadosVGE[i,13]+ (aDadosVGE[i,12]/2) .and. vendTercei != 'S'
				aDadosVGE[i,9] := 0
			endif
		next i
		//VENDA INCREMENTO
		for i:=1 to len(aDadosIGE)
			...
			//REGRA VENDAS MENOR QUE 20 NAO COMISSIONA
			if 20 > aDadosIGE[i,11] + aDadosIGE[i,13]+ (aDadosIGE[i,12]/2) .and. vendTercei != 'S'
				aDadosIGE[i,8] := 0
			endif
		next i
		//VENDA RENOVAวรO
		for i:=1 to len(aDadosRGE)
			...
			TEMP->(DBCLOSEAREA())
			//REGRA VENDAS MENOR QUE 20 NAO COMISSIONA
			if 20 > aDadosRGE[i,12] + aDadosRGE[i,14]+ (aDadosRGE[i,13]/2) .and. vendTercei != 'S'
				aDadosRGE[i,8] := 0
			endif
		next i
	endif
