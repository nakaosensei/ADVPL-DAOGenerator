...
Do While !WORK->(EOF())
   ...
   //Embarque Autorizado
   If (mv_par04 == 1 .And. Str(mv_par04) # WORK->WK_OK_SHP) .Or. (mv_par04 == 2 .And. Str(mv_par04) # WORK->WK_OK_SHP)
      WORK->(dbDelete())
      WORK->(dbSkip())
      LOOP
   EndIf
   
   If (mv_par05 == 1 .And. Empty(WK_DT_IEA)) .Or. (mv_par05 == 2 .And. !Empty(WK_DT_IEA)) //Instr.Embarque Emitida
      WORK->(dbDelete())
      WORK->(dbSkip())
      LOOP
   EndIf
...
Define MsDialog oDLG Title "Relatorio de Invoices Antecipadas" From 0,0 To 80,450 Of oMainWnd Pixel
   *
