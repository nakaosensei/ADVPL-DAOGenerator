...
lAtuDtCont:= lScheduled .OR. MsgYesNo(STR0001) //"Integração Efetiva ?"
...
DO WHILE .NOT. SWB->(EOF()) .AND. SWB->WB_CA_DT <= (dFimEncerra + 5) ;  && pagto no mes
   ...
   SW9->(DbSetOrder(01))
   //TDF 06/12/2010 - ACRESCENTA O HAWB NA CHAVE DE BUSCA
   SW9->(DbSeek(xFilial()+SW8->W8_INVOICE+cForn+EICRetLoja("SW8", "W8_FORLOJ")+SW8->W8_HAWB))
   ...
   lDI100 := (DI_Ctb->(DBSEEK(SWB->WB_HAWB)) .or. EC2->(DBSEEK(XFILIAL("SWB")+SWB->WB_HAWB)))
...
//    ENDCASE
      ...
      Do While SWB->(!EOF()) .And. SWB->WB_FILIAL == xFilial("SWB") .And.;
          ...
          If SWB->WB_INVOICE == SW9->W9_INVOICE .And. SWB->WB_TIPOREG=="1"
             ...
             If lExisteECF .and. ECF->(DBSEEK(cFilECF+"IMPORT"+SW9->W9_FORN+SWB->WB_INVOICE+AVKEY(SYS->YS_CC,"ECF_IDENTC")+"608"+SWB->WB_LINHA+"PR")) // Nick 09/10/06
...
IF SWB->(DBSEEK(cFil_SWB+SW6->W6_HAWB+"D")) .And. !Empty(SW6->W6_DT_EMB) //.And. Empty(SW6->W6_DT_ENCE) // Nick 25/05/06 !Empty(SW6->W6_DT_ENCE) // Tipo de registro de pagamento antecipado para o embarque/desembaraco
