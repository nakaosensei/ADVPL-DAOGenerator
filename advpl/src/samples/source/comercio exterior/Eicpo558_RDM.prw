...
WHILE .T.
  ...
  DEFINE MSDIALOG oDlg TITLE STR0001 FROM 8, 0 TO  30, 80 //"Emissão da Comercial Invoice"
...
AVPRINT oPrn NAME STR0009 //"Commercial Invoice"
  ...
  IncProc( STR0010 ) //"Imprimindo..."
    ...
    Do while ! SW9->(EOF()) .AND. SW9->W9_FILIAL == xFilial("SW9") .AND. SW9->W9_HAWB == cHouse    
	   ...
	   If SW8->(!DbSeek(xFilial("SW8")+cHouse+SW9->W9_INVOICE+SW9->W9_FORN+EICRetLoja("SW9","W9_FORLOJ")))
          Exit
       EndIf
       
       Do while ! SW8->(EOF()) .AND. SW8->W8_FILIAL == xFilial("SW8") .AND. SW8->W8_INVOICE == SW9->W9_INVOICE .AND.;
                 (SW8->W8_FORN == SW9->W9_FORN .AND. IIF(EICLoja(),SW8->W8_FORLOJ == SW9->W9_FORLOJ,.T.))
          ...
          SYG->(DBSEEK(xFilial("SYG")+SW2->W2_IMPORT+SW8->W8_FABR+EICRetLoja("SW8","W8_FABLOJ")+SW8->W8_COD_I))
          mDescri := ''
          ...
          IF ASCAN(aVetor,SW8->W8_FABR+EICRetLoja("SW8","W8_FABLOJ")) == 0
             AADD(aVetor,SW8->W8_FABR+EICRetLoja("SW8","W8_FABLOJ"))
          ENDIF
       ...
       oPrn:Say( nLinha   ,2180 ,trans(SW9->W9_FRETEIN,AVSX3("W9_FRETEIN",6)), aFontes:TIMES_NEW_ROMAN_12 ,,,,1)  //TRP-18/02/08  
       //nFob_Tot := nFob_Tot + SW6->W6_FRETEIN * nPari_1
       nFob_Tot := nFob_Tot + SW9->W9_FRETEIN  //TRP-18/02/08
       PO558VerFim(2)
