...
WHILE .T.
   ...
   nOpcao:=0
   oDlgTelaPrincipal:= oSend( TDialog(), "New", 120, 100, 250, 450,;
...
oPrn:Say( Linha, 105, MEMOLINE(SY3->Y3_DESC,12,1), aFontes:TIMES_NEW_ROMAN_08_NEGRITO )
...
oPrn:Say( Linha, 570,BUSCA_UM(SW3->W3_COD_I+SW3->W3_FABR +SW3->W3_FORN,SW3->W3_CC+SW3->W3_SI_NUM,IF(EICLOJA(),SW3->W3_FABLOJ,""),IF(EICLOJA(),SW3->W3_FORLOJ,""))                            , aFontes:TIMES_NEW_ROMAN_08_NEGRITO )
...
n2 := MLCOUNT( SUBSTR(ALLTRIM(SY3->Y3_DESC),1,LEN(SY3->Y3_DESC)-12), 12 )
...
FOR i:=2 TO n1 + 1   // Soma um para bater o ultimo.
    ...
    IF !EMPTY( MEMOLINE(SY3->Y3_DESC,12,i) )
       oPrn:Say( Linha, 105, MEMOLINE(SY3->Y3_DESC,12,i), aFontes:TIMES_NEW_ROMAN_08_NEGRITO )
       lPulaLinha := .T.
    ENDIF
