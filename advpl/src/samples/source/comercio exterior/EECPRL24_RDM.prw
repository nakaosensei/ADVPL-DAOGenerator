      ...
      While xFilial("EX6") == EX6->EX6_FILIAL    ;
         ...
         If lNovaRotina 
            ...
            If !Empty(EX6->EX6_DTAPRO)
               DET->DTAPROV := transform(EX6->EX6_DTAPRO,avsx3("EX6_DTAPRO",AV_PICTURE)) 
            Else
               DET->DTAPROV := "-"
            Endif
         Endif
...
Begin Sequence
   ...
   DEFINE MSDIALOG oDlg TITLE cTitRpt FROM 1,1 TO 203-(n*2),273-(m*2) OF oMainWnd PIXEL
...
#IFDEF TOP
   If TCSRVTYPE() = "AS/400"
      ...
      If Empty((cAlias)->EX5_DTAPRO)
         DET->DTAPROV  := "-"
      Else
         DET->DTAPROV  := TRANSFORM(DTOC((cAlias)->EX5_DTAPRO) ,AVSX3("EX5_DTAPRO" ,AV_PICTURE))  
      EndIf  
   Else    
