...
SW5->(dbSeek(xFilial()+cPGI_Num))
  ...
  While ! SW5->(Eof()) .And. SW5->W5_FILIAL == cFilSW5 .And.;
   ...
   For i := 1 To Work1->(FCount())
      If Work1->(FieldName(i)) $ "RECNO,FLAG,DELETE,W5_DESC_P,W5_FABR_N,W5_FORN_N,TRB_ALI_WT,TRB_REC_WT"
         LOOP
      Endif
   ...
   SA2->(DbSeek(xFilial("SA2")+Work1->W5_FABR+EICRetLoja("Work1","W5_FABLOJ")))
   Work1->W5_FABR_N := SA2->A2_NREDUZ
   
   SA2->(DbSeek(xFilial("SA2")+Work1->W5_FORN+EICRetLoja("Work1","W5_FORLOJ")))
