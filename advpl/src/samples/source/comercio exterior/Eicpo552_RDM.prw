...
AADD(TB_Campos,{"YT_COD_IMP","","Importador"}) //Importador
...
WHILE .T.
  ...
  @ 29.0,5.0 SAY "Consolida Itens por Dt. Embarque?" SIZE 90,8 of oDlg Pixel 
  ...
  IF Work1->(LASTREC()) > 0
     DO WHILE .T.
        ...
        If (Select ("Work_men") # 0) //- BHF - 11/08/08 - Tratamento de verificação da work.
           @ 05,(oDlg:nClientWidth-326)/2-30 BUTTON "Salva Modelo"  SIZE 36,13 ACTION PO552GravaMsg() OF oPanel PIXEL  //TRP-17/06/08
        EndIf
...
WHILE ! Work1->(EOF())
  IF cPOMarcado == Work1->W3_PO_NUM
     ...
     IF !EMPTY(cNewMarca)
        AADD(aMarcados,{Work1->W3_PO_NUM, Work1->WKREC_SW5, Work1->A2_COD, Work1->YT_COD_IMP})
     ENDIF
  ELSE
     Work1->WKFLAG :=SPACE(02)
  ENDIF
...
Local mNCM
Local aItens   := {}, cOrder := ""
Local nItens   := 0 , nTotal:= 0 ,nTotalFOB := 0, nInland:= 0 , nPacking:= 0 , nFreight:= 0, nDescont:= 0
I         := 0
cAux      := ''
W         := 0
...
AVPRINT oPrn NAME STR0043 //"EmissÒo de InstruþÒo de Embarque / Shipping Instructions"
   ...
   AVPAGE
        ...
        If Substr(cOrder, Len(cOrder),1) == '/'
           cOrder := Substr(cOrder,1,Len(cOrder)-2)
        EndIf
        ...
        IF nIdioma==1
           ...
           p2 := "PLEASE FOLLOW THE INSTRUCTIONS:"
        ...
        DO WHILE ! Work1->(EOF())
           ...
           If cEmbarque == cConsolida
              If Empty(aItens)    
                 Aadd(aItens,{SW5->W5_COD_I, SW5->W5_QTDE , mDescri, mNCM, SW5->W5_PRECO, SW5->W5_QTDE * SW5->W5_PRECO, DATA_ITEM, SW5->W5_DT_EMB })
              Else             
                 If (nItem:=aScan(aItens, {|x| x[1] == SW5->W5_COD_I .AND. x[8] == W5_DT_EMB})) == 0 
                    Aadd(aItens,{SW5->W5_COD_I, SW5->W5_QTDE , mDescri, mNCM,  SW5->W5_PRECO, SW5->W5_QTDE * SW5->W5_PRECO, DATA_ITEM, SW5->W5_DT_EMB }) //WFS
                 Else              
                    AItens[nItem][2] += SW5->W5_QTDE
                    AItens[nItem][6] += SW5->W5_QTDE * SW5->W5_PRECO
                 EndIf  
              EndIf   
           Else
              Aadd(aItens,{SW5->W5_COD_I, SW5->W5_QTDE , mDescri, mNCM, SW5->W5_PRECO, SW5->W5_QTDE * SW5->W5_PRECO, DATA_ITEM, SW5->W5_DT_EMB })
        ...
        //ASK 15/10/2007 - Tratamento para Embarque com mais de um P.O.
           ...
           oPrn:Say( nLinha    ,490  ,memoline(aItens[j][3],18,1),ofont1)
           ...
           FOR W:=2 TO MLCOUNT(mDescri,18)
              IF ! EMPTY(ALLTRIM(memoline(mDescri,18,W)))
                 Parte2 := 3
			     PO552VERFIM()
			     BateTraco()
                 oPrn:Say( nLinha,490  ,memoline(mDescri,18,W), aFontes:TIMES_NEW_ROMAN_12)
                 nLinha := nLinha + 50
              ENDIF
           NEXT
           nLinha := nLinha + 10
