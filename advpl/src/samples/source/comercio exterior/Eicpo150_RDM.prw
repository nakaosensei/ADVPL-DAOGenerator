...
oSend( oPrn, "Say",  Linha, 630, SA2->A2_NOME + Space(2) + Alltrim(IF(EICLOJA(),"Loja:" + Alltrim(SA2->A2_LOJA),"")) , aFontes:TIMES_NEW_ROMAN_12 )
Linha := Linha+50
...
IF nIdioma == INGLES
   oSend( oPrn, "Say",  Linha, 630, MEMOLINE(cTerms,AVSX3("Y6_VM_DESI",3),1)      , aFontes:TIMES_NEW_ROMAN_12 )
ELSE
   oSend( oPrn, "Say",  Linha, 630, MEMOLINE(cTerms,AVSX3("Y6_VM_DESP",3),1)      , aFontes:TIMES_NEW_ROMAN_12 )
ENDIF
Linha := Linha+50
...
// oSend( oPrn, "Say",  Linha, 480, AV_Justifica(MEMOLINE( cDescrItem,nTam ,1 )),aFontes:COURIER_NEW_10_NEGRITO )
   oSend( oPrn, "Say",  Linha, 480, MEMOLINE( cDescrItem,nTam ,1 ),aFontes:COURIER_NEW_10_NEGRITO )
ENDIF
//Acb - 27/09/2010 - retornado posicionamento 1500 pois o nome do produtos e o nome do fabricante estava sobrepondo um ao outro.
oSend( oPrn, "Say",  Linha,/*1250*/1500, AllTrim(SA2->A2_NREDUZ) + Space(2) + IF(EICLOJA(),"Loja:" + Alltrim(SA2->A2_LOJA),"") ,aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 )
