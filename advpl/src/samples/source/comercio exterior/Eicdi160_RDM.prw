...
cHouse := SW6->W6_HAWB
  ...
  /*+--------------------------------------------------------------+
   ...
   cTitulo:=STR0001 //"EmissÒo da InstruþÒo de Despacho"
    ...
    @ 45, 60 BUTTON STR0002 SIZE 34,11 ACTION (nOpcao:=2,oSend(oDlg,'End')) of oDlg Pixel //"Mensagens"
...
//AADD(aVetor,{SWP->WP_REGIST,SW2->W2_MOEDA,SW7->W7_QTDE*SW7->W7_PRECO,SW4->W4_REGIMP,SY8->Y8_DES,SY8->Y8_REG_TRI})
        ...
        DI160VerFim(0)  //CCM - 12/05/09 - Alteração para não imprimir o nome da moeda quando valor = 0. 
        oPrn:Say( Linha:=Linha+90 , 50  ,STR0036 + If(SW6->W6_VLFREPP > 0, " " + SW6->W6_FREMOED+" "+;
                  Trans(SW6->W6_VLFREPP,AVSX3("W6_VLFREPP",6)),""),aFontes:TIMES_NEW_ROMAN_12) //TRP-10/10/07

        oPrn:Say( Linha:=Linha+50 , 50  ,STR0037 + If(SW6->W6_VLFRECC > 0, " "+ SW6->W6_FREMOED+" "+;
                  Trans(SW6->W6_VLFRECC,AVSX3("W6_VLFRECC",6)),""),aFontes:TIMES_NEW_ROMAN_12) //TRP-10/10/07
        
        
        oPrn:Say( Linha:=Linha+50 , 50  ,STR0038 + If(SW6->W6_VLFRETN > 0, " "+ SW6->W6_FREMOED+" "+;
