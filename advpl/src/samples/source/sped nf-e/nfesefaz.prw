...
Local nPercProd 	:= 0   // Porcentual do produto da Nf sobre cupom  
		...
		Else
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD2
						...
						D2_CLASFIS,D2_PRCVEN,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_DESCZFC,D2_DESCZFP,D2_LOTECTL,D2_NUMLOTE,D2_ICMSRET %Exp:cField%
						FROM %Table:SD2% SD2
						WHERE
...
//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
				...
				nPercProd := (((cAliasSD2)->D2_PRUNIT/ SF2->F2_VALMERC) * 100) 
				 
				If lNfCup 
					nDesconto := NoRound((SF2->F2_DESCONT * nPercProd ) / 100)
				Else
				...
				aadd(aProd,	{Len(aProd)+1,;
					...
					SB1->B1_UM,;
					(cAliasSD2)->D2_QUANT,;
					IIF(!(cAliasSD2)->D2_TIPO$"IP",Iif(!(cAliasSD2)->D2_TIPO$"D",(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR,(cAliasSD2)->D2_TOTAL),0),;
					...
					(cAliasSD2)->D2_VALFRE,;
					(cAliasSD2)->D2_SEGURO,;
					(nDesconto)/*(cAliasSD2)->D2_DESCZFR)*/,;
					IIF(!(cAliasSD2)->D2_TIPO$"IP", IIf(!(cAliasSD2)->D2_TIPO $ "D", (cAliasSD2)->D2_PRCVEN+((nDesconto+(cAliasSD2)->D2_DESCZFR)/(cAliasSD2)->D2_QUANT), (cAliasSD2)->D2_PRCVEN), 0),;
				...
				If SFT->(FieldPos("FT_CSTPIS")) > 0 .And. SFT->(FieldPos("FT_CSTCOF")) > 0
					...
					If MsSeek(xFilial("SFT")+"S"+SF2->F2_SERIE+SF2->F2_DOC+SF2->F2_CLIENTE+SF2->F2_LOJA+PadR((cAliasSD2)->D2_ITEM,4)+(cAliasSD2)->D2_COD)
						...
						IF Empty(aPis[Len(aPis)]) .And. SFT->FT_CSTPIS$"09,06,04,99"
							aTail(aPisAlqZ):= {SFT->FT_CSTPIS}
						EndIf
						IF Empty(aCOFINS[Len(aCOFINS)]) .And. SFT->FT_CSTCOF$"09,06,04,99"
				...
				Else
					...
					IF Empty(aPis[Len(aPis)]) .And. SF4->F4_CSTPIS$"09,06,04,99"
						aTail(aPisAlqZ):= {SF4->F4_CSTPIS}		
					EndIf
					IF Empty(aCOFINS[Len(aCOFINS)]) .And. SF4->F4_CSTCOF$"09,06,04,99"
		...
		Else				
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD1			
					SELECT D1_FILIAL,D1_DOC,D1_SERIE,D1_FORNECE,D1_LOJA,D1_COD,D1_ITEM,D1_TES,D1_TIPO,D1_NFORI,D1_SERIORI,D1_ITEMORI,D1_CF,D1_QUANT,D1_TOTAL,D1_VALDESC,D1_VALFRE,D1_SEGURO,D1_DESPESA,D1_CODISS,D1_VALISS,D1_VALIPI,D1_ICMSRET,D1_VUNIT,D1_CLASFIS,D1_VALICM,D1_TIPO_NF,D1_PEDIDO,D1_ITEMPC,D1_VALIMP5,D1_VALIMP6,D1_BASEIRR,D1_VALIRR,D1_LOTECTL,D1_NUMLOTE,D1_CUSTO,D1_ORIGLAN,D1_DESCICM %Exp:cField%
					FROM %Table:SD1% SD1
					WHERE
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				Else
					...
					If Alltrim( (cAliasSD1)->D1_ORIGLAN ) $ "D-DP" .And. Alltrim( (cAliasSD1)->D1_TIPO ) == "C"
						dbSelectArea("SF8")
						dbSetOrder(1)
...
IF ExistBlock("PE01NFESEFAZ")                   
	...
	aParam := {aProd,cMensCli,cMensFis,aDest,aNota,aInfoItem}
...
If !Empty(aNota)
	...
	For nX := 1 To Len(aProd)
		cString += NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],aMed[nX],aArma[nX],aveicProd[nX],aDI[nX],aAdi[nX],aExp[nX],aPisAlqZ[nX],aCofAlqZ[nX],aAnfI[nX],cTipo,cVerAmb, aComb[Nx],@cMensFis,aCsosn[Nx],aPedCom[nX],aNota[5])
	Next nX
...
Static Function NfeItem(aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aAnfI,cTipo,cVerAmb,aComb,cMensFis,cCsosn,aPedCom,cF2Tipo)
...
If cF2Tipo $ "D"
	cString += NfeTag("<vDesc>", ConvType(0, 15, 2))
Else
...
Else
	...
	If lSimpNac .And. ( !Empty(aPisAlqZ) .And. aPisAlqZ[01] == "99" )
...
Else
	...
	If lSimpNac .And. ( !Empty(aCofAlqZ) .And. aCofAlqZ[01] == "99" )
