...
±±ºPROGRAMA  ³LIGFIN06 ºAUTOR  ³ROBSON ADRIANO 		   ºDATA ³ 22/07/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDESC.     ³ ROTINA PARA AGENDA NO SCHEDULE, PARA VERIFICAR DUPLICATAS  º±±
±±º          ³ EM ABERTO E ENVIAR NOTIFICACOES E BLOQUEIOS DE SERVICOS    º±±
...
User Function LIGFIN06()
	...
	Local _cAssunto2 := "Fatura Ligue Telecom: Email de Notificação"
...
//	PREPARE ENVIRONMENT EMPRESA "01" FILIAL "LG01" MODULO "FAT"
    ...
    _cMsgDesP := "Desligamento parcial por falta de pagamento de serviço"
    _cMsgDesT := "Desligamento total por falta de pagamento de serviço"
...
//		cQuery += " AND SE1.E1_ULIBFAT = 'S'"
	...
	WHILE !EOF() 
		...
		//BLOQUEAR CLIENTES
  		...
  		ENDIF
   		...
   		IF  DATE() - STOD(TSE1->E1_VENCTO) = _nPriNot
   			...
   			IF (TSE1->E1_PREFIXO = 'CTR')
   				_msgFim := msg2Fim()
   			ELSE
   				_msgFim += "<p>Se o pagamento já foi efetivado desconsidere essa mensagem.</p>"
   			ENDIF
  		...
  		//ENVIAR EMAIL DE 5 EM 5 DIAS APÓS O BLOQUEIO DO CLIENTE
  		...
  		//ENVIAR EMAIL 90 APÓS VENCIMENTO
  		IF DATE() - STOD(TSE1->E1_VENCTO) >= _nOitNot .AND. DATE() - STOD(TSE1->E1_VENCTO) <= _nOitNot + 3
   				...
   				_CMsgCanc += _MSG
   			   //U_LIGGEN03(_email3,"","","Fatura vencida à 90 dias. Cancelamento",_MSG,.T.,"")
   				U_SaveSE1(8)	
  		ENDIF
		...
		ENDIF
   		...
   		IF !EMPTY(TSE1->E1_UDTBLS)	
	   		...
	   		// Enviar email 1 dia antes do serviço ser bloqueado.
	...
	IF !EMPTY (_CMsgSPC)
   		U_LIGGEN03(_email2,"","","Fatura vencida à 30 dias.",_CMsgSPC,.T.,"")
	ENDIF 
	
	IF !EMPTY (_CMsg60Dias)
   		U_LIGGEN03(_email2,"","","Fatura vencida à 60 dias.",_CMsgSPC,.T.,"")
	ENDIF 
	
	IF !EMPTY (_CMsgCanc)
	   U_LIGGEN03(_email3,"","","Fatura vencida à 90 dias. Cancelamento",_CMsgCanc,.T.,"")
	ENDIF  
	...
	IF !EMPTY (_CMsgBloq)
	   U_LIGGEN03(_email2,"","","Clientes bloqueados pela rotina Automática",_CMsgBloq,.T.,"")
	ENDIF 
	  
	IF !EMPTY (_CMsgSYate)
	   U_LIGGEN03(_email2,"","","Clientes sem código do yate",_CMsgSYate,.T.,"")
	ENDIF   
...
_cMsg += "<p>Informamos que em " + Transform(STOD(_DVenc),"@E") + " vencerá/vencerão o(s) título(s) originado(s) pelo(s) serviço(s) prestado(s) à V.Sa</p>"
return _cMsg

Static Function msg1Fim()
Local _cMsg := ""
_cMsg += "<p>Caso não tenha recebido os documentos necessários para programação do(s) pagamento(s),"
...
_cMsg += "<p>Se necessário, favor informar abaixo novos contatos para recebimento do faturamento:</p>"
_cMsg += "<p>Nome: </p>"
_cMsg += "<p>Departamento: </p>"
_cMsg += "<p>Contato: </p>"
_cMsg += "<p>Email: </p>"
_cMsg += "<p>Caso já tenha recebido os devidos documentos (nota e boleto) e tais valores já estejam em vossa programação de pagamento, por favor desconsiderar este comunicado</p>"
_cMsg += "<p>Colocamo-nos à disposição para solucionar quaisquer dúvidas.</p>"
return _cMsg
...
Local _cMsg := ""
_cMsg += "<p>Efetue o pagamento evite a suspensão de seu(s) serviço(s), transcorrido o prazo de 10 (DEZ) dias do vencimento da fatura original, 
_cMsg += " irá ocorrer a suspensão do(s) serviço(s) contratado(s).</p>"
_cMsg += "<p><b>Observações: </b></p>"
_cMsg += "<p>Se o pagamento já foi efetivado desconsidere essa mensagem.</p>"
_cMsg += "<p>Caso não tenha recebido os documentos necessários para programação do(s) pagamento(s),"
...
Local _cMsg := ""
_cMsg += "<p><b>SOLICITAMOS, que através da presente notificação quite integralmente suas pendência. 
_cMsg += " Transcorrido o prazo de 10 (DEZ) dias do vencimento da fatura original, irá ocorrer a suspensão dos serviços contratados.</b></p>"
_cMsg += "<p><b>Observações: </b></p>"
_cMsg += "<p>Se o pagamento já foi efetivado desconsidere essa mensagem.</p>"
_cMsg += "<p>Caso não tenha recebido os documentos necessários para programação do(s) pagamento(s),"
_cMsg += "	 favor entrar em contato conosco pelo telefone (44) 3810-0000 e/ou e-mail indicado abaixo:</p>"
...
IF !EMPTY(ALLTRIM(_cIdYate))
	...
	_MSG := "<p>Clientes Bloqueados</p>"
   	...
   	_CMsgBloq += _MSG
ELSE
	_MSG := "<p>Cliente sem código no yate.</p>"
   	_MSG += "<p>Código : " + TSE1->E1_CLIENTE + " Nome : " + TSE1->E1_NOMCLI + " Fatura : " + TSE1->E1_PREFIXO + " " + TSE1->E1_NUM + " "+ TSE1->E1_PARCELA + "</p>"
   	_MSG += "<p>Vencimento : "+Transform(STOD(TSE1->E1_VENCTO),"@E") +"</p>"
   	_MSG += "<p>Valor : "+Transform(TSE1->E1_VALOR,"@E 9,999,999.99",) +"</p><hr>" 
...
IF dbseek(xFilial("ADA")+ _nCtr)	
	IF ADA->ADA_MSBLQL == '2' .AND. ADA->ADA_UCANCE <> '3'
		RECLOCK("ADA",.F.)
			ADA->ADA_UCANCE := '3'		
			ADA->ADA_UDSCAN := "Contrato cancelado devido a falta de pagamento. Via rotina de retorno bancário."	
		MSUNLOCK()
	ENDIF
ENDIF
