...
//								_os := U_LIGTEC01(ADB->ADB_CODPRO,ADB->ADB_CODCLI,ADB->ADB_LOJCLI,_condi,_cOcoIns,_cSerIns,ADB->ADB_QUANT,ADB->ADB_PRCVEN,_cNumSer,ADB->ADB_UMSG,_cAtInst,"1",ALLTRIM(ADA->ADA_NUMCTR))
							...
							aAdd(aDados,{ADB->ADB_CODPRO,ADB->ADB_CODCLI,ADB->ADB_LOJCLI,_condi,_cOcoIns,_cSerIns,ADB->ADB_QUANT,ADB->ADB_PRCVEN,_cNumSer,ADB->ADB_UMSG,_cAtInst,"1",ALLTRIM(ADA->ADA_NUMCTR),ADB->ADB_ITEM,"INICIO VIGENCIA","",ADA->ADA_UOBS,ADA->ADA_UCLIAS})
/*								if !empty(_os)
									dbselectarea("SZ2")
									reclock("SZ2",.t.)
...
//					ENDIF //COMENTADO POR DANIEL EM 20/12/16
		...
		if(isLnkFull = .T.) 
			IF !EMPTY(_CosInclu) 
				U_TEC02F(_CosInclu , getmv("MV_UEMLNOC"))
			ENDIF
		endif
...
/*
* Enviar email com base na OS
* Por padrão deve estar posicionado no ADA
*/
User Function TEC02F(_pOS, _cEmlNoc)
Local _area := getarea()		
...
Local _aAB7 := AB7->(getarea())	
	...
	DbSelectArea("AB6")	 
   	DBGoTop()     
	IF DBSEEK(xFilial() + _pOS)
		_MSG := ""
		
		IF !EMPTY(AB6->AB6_UNUMCT)
			_MSG += "<p>Contrato " + AB6->AB6_UNUMCT + "</p>"
		ENDIF
		
		 _nome := POSICIONE("SA1",1,XFILIAL( "SA1" )+AB6->AB6_CODCLI+AB6->AB6_LOJA,"A1_NOME")   
	   	
	   	_MSG += "<p>Código : " + AB6->AB6_CODCLI + " Nome : " + _nome + "</p>"
	   	_MSG += "<p>OS : " + _pOS + "</p>"
	   	_MSG += "<p>Atendente : " + AB6->AB6_ATEND + "</p>"
   	ENDIF
   	
   	DbSelectArea("AB7")	 
   	DBGoTop()     
	IF DBSEEK(xFilial() + _pOS + "01")  //Move o ponteiro para o primeiro item da OS em questão de AB7
		WHILE (!EOF() .AND. AB7->AB7_FILIAL+AB7->AB7_NUMOS == xFilial() + _pOS)
			
			IF !EMPTY(AB7->AB7_NRCHAM)
			 	_Solic  := POSICIONE("AB1",1,XFILIAL( "AB1" )+AB7->AB7_NRCHAM,"AB1_ATEND")   
			 	_MSG += "<p>* Solicitante : " + _Solic + "</p>"
			ENDIF
			
			_produto := POSICIONE("SB1",1,XFILIAL( "SB1" )+AB7->AB7_CODPRO,"B1_DESC")   
			_MSG += "<p>* Item : " + AB7->AB7_ITEM + " Código : " + AB7->AB7_CODPRO +  " Produto "  + _produto  + "</p>"
			_MSG += "<p>** Obs:"+ MSMM(AB7->AB7_MEMO1) +"</p>"
		
			DBSKIP()	
		ENDDO
	ENDIF	
   	 	
    U_LIGGEN03(_cEmlNoc,"","robson.adriano@liguetelecom.com.br","Ordem de serviço criada",_MSG,.T.,"")
